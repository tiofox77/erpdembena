<div class="min-h-screen bg-gradient-to-br from-slate-50 via-amber-50 to-orange-50">
    <div class="bg-gradient-to-r from-amber-600 via-yellow-600 to-orange-700 shadow-xl">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h1 class="text-2xl font-bold text-white">{{ trans('messages.advances_report') }}</h1>
                        <p class="text-amber-100 text-sm">{{ \Carbon\Carbon::parse($startDate)->format('d/m/Y') }} - {{ \Carbon\Carbon::parse($endDate)->format('d/m/Y') }}</p>
                    </div>
                </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <div class="bg-white rounded-xl shadow-sm border p-4">
                <p class="text-xs text-gray-500 mb-2">{{ trans('messages.total_advances') }}</p>
                <p class="text-2xl font-bold text-gray-900">{{ number_format($summary['total_advances']) }}</p>
            </div>
            <div class="bg-white rounded-xl shadow-sm border p-4">
                <p class="text-xs text-gray-500 mb-2">{{ trans('messages.total_amount') }}</p>
                <p class="text-2xl font-bold text-amber-600">{{ number_format($summary['total_amount'], 0, ',', '.') }}</p>
                <p class="text-xs text-gray-500">AOA</p>
            </div>
            <div class="bg-white rounded-xl shadow-sm border p-4">
                <p class="text-xs text-gray-500 mb-2">{{ trans('messages.remaining') }}</p>
                <p class="text-2xl font-bold text-orange-600">{{ number_format($summary['total_remaining'], 0, ',', '.') }}</p>
                <p class="text-xs text-gray-500">AOA</p>
            </div>
            <div class="bg-white rounded-xl shadow-sm border p-4">
        ...
        <div class="bg-white rounded-xl shadow-sm border overflow-hidden">
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap text-sm">{{ \Carbon\Carbon::parse($advance->request_date)->format('d/m/Y') }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-gray-900">{{ $advance->employee->full_name }}</div>
                                <div class="text-xs text-gray-500">{{ $advance->employee->department->name ?? '-' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-bold text-amber-600">{{ number_format($advance->amount, 0, ',', '.') }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-center text-sm">{{ $advance->installments }}x</td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm text-orange-600">{{ number_format($advance->remaining_amount, 0, ',', '.') }}</td>
            ...
            <div class="px-6 py-4 border-t">{{ $advances->links() }}</div>
        </div>
    </div>
</div>
