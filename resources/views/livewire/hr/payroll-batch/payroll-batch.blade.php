...
<div class="min-h-screen bg-gray-50">
            ...
            <div class="bg-gradient-to-br from-purple-600 via-purple-700 to-indigo-800 px-6 py-8 text-white flex-shrink-0">
                <div class="w-full flex items-center justify-between">
                    <div class="flex items-center space-x-4">
                        ...
                        <div>
                            <h1 class="text-3xl font-bold">{{ __('payroll.page_title') }}</h1>
                            <p class="text-purple-100 mt-1">{{ __('payroll.page_description') }}</p>
                        </div>
                    </div>
                    <div class="flex items-center space-x-3">
                        ...
                        >
                            ...
                            <span>{{ __('payroll.create_new_batch') }}</span>
                        </button>
                        @endif
                        <button
                        ...
                        >
                            ...
                            <span>{{ __('payroll.clear_filters') }}</span>
                        </button>
                    </div>
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    <div class="bg-gradient-to-br from-blue-50 to-blue-100 p-6 rounded-xl border border-blue-200 hover:shadow-lg transition-all duration-300">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-blue-600 font-medium text-sm">{{ __('payroll.total_batches') }}</p>
                    ...
                    <div class="bg-gradient-to-br from-yellow-50 to-yellow-100 p-6 rounded-xl border border-yellow-200 hover:shadow-lg transition-all duration-300">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-yellow-600 font-medium text-sm">{{ __('payroll.processing_batches') }}</p>
                    ...
                    <div class="bg-gradient-to-br from-green-50 to-emerald-100 p-6 rounded-xl border border-green-200 hover:shadow-lg transition-all duration-300">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-green-600 font-medium text-sm">{{ __('payroll.completed_batches') }}</p>
                    ...
                    <div class="bg-gradient-to-br from-purple-50 to-purple-100 p-6 rounded-xl border border-purple-200 hover:shadow-lg transition-all duration-300">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-purple-600 font-medium text-sm">{{ __('payroll.approved_batches') }}</p>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        ...
                        <div>
                            ...
                            >
                                ...
                                <option value="draft">{{ __('payroll.status_draft') }}</option>
                                <option value="ready_to_process">{{ __('payroll.status_ready_to_process') }}</option>
                                <option value="processing">{{ __('payroll.status_processing') }}</option>
                                <option value="completed">{{ __('payroll.status_completed') }}</option>
                                <option value="failed">{{ __('payroll.status_failed') }}</option>
                                <option value="approved">{{ __('payroll.status_approved') }}</option>
                                <option value="paid">{{ __('payroll.status_paid') }}</option>
                            </select>
                        </div>
                        <div>
                            <label for="department_filter" class="block text-sm font-medium text-gray-700 mb-2">
                                <i class="fas fa-building text-gray-400 mr-1"></i>
                                {{ __('payroll.department') }}
                            </label>
                            <select
                                ...
                                class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-all duration-200"
                            >
                                <option value="">{{ __('payroll.all_departments') }}</option>
                        ...
                        <div>
                            ...
                            <select
                                ...
                                class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-all duration-200"
                            >
                                <option value="">{{ __('payroll.all_periods') }}</option>
                                    ...
                                    <div class="lg:col-span-2">
                                        <div class="space-y-2">
                                            <div class="flex items-center space-x-2 text-sm">
                                                ...
                                                <span class="text-gray-700 font-medium">{{ $batch->total_employees }} {{ __('payroll.employees_text') }}</span>
                                            </div>
                                            <div class="flex items-center space-x-2 text-sm">
                                                <i class="fas fa-check-circle text-green-500"></i>
                                                <span class="text-gray-700">{{ $batch->processed_employees }} {{ __('payroll.processed_text') }}</span>
                                            </div>
                                            @if($batch->total_employees > 0)
                                                ...
                                                <p class="text-xs text-gray-500">{{ $batch->progress_percentage }}% {{ __('payroll.progress_complete') }}</p>
                                            @endif
                                        </div>
                                    </div>
                                    ...
                                    <div class="lg:col-span-3">
                                        <div class="bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg p-3 border border-green-200">
                                            <div class="grid grid-cols-2 gap-3">
                                                <div class="text-center">
                                                    <p class="text-xs text-green-600 font-medium">{{ __('payroll.gross_value') }}</p>
                                                    <p class="text-sm font-bold text-green-700">{{ number_format($batch->total_gross_amount, 0) }}</p>
                                                </div>
                                                <div class="text-center">
                                                    <p class="text-xs text-green-600 font-medium">{{ __('payroll.net_value') }}</p>
                                    ...
                                    <div class="lg:col-span-1">
                                        <div class="flex justify-center">
                                            ...
                                            ">
                                                ...
                                                "></div>
                                                {{ __('payroll.status_' . $batch->status) }}
                                            </span>
                                        </div>
                                    </div>
                        ...
                        @empty
                            <div class="text-center py-12">
                                <div class="flex flex-col items-center space-y-4">
                                    ...
                                    <div>
                                        <h3 class="text-lg font-medium text-gray-900">{{ __('payroll.no_batches_found') }}</h3>
                                        <p class="text-gray-500 mt-1">{{ __('payroll.create_first_batch') }}</p>
                                    </div>
                                    @if($this->canProcessBatch())
                                        <button
                                            wire:click="openBatchModal"
                                        ...
                                        >
                                            ...
                                            <span>{{ __('payroll.create_first_batch_button') }}</span>
                                        </button>
                                    @endif
                                </div>
                            </div>
    ...
    @include('livewire.hr.payroll-batch.modals._edit-item-modal-complete')
</div>
