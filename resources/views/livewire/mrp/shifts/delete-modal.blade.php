                ...
                <div class="p-6">
                    <div class="text-center">
                        ...
                        <h3 class="text-lg leading-6 font-medium text-gray-900 mb-4">
                            {{ __('messages.confirm_delete_shift') }}
                        </h3>
                        
                        @if($shiftToDelete)
                            <div class="mt-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
                                <div class="flex items-center mb-2">
                                    <div class="w-4 h-4 rounded-full mr-2" style="background-color: {{ $shiftToDelete->color_code }}"></div>
                                    <span class="text-lg font-medium text-gray-700">{{ $shiftToDelete->name }}</span>
                                </div>
                                <p class="text-sm text-gray-500">{{ $shiftToDelete->start_time }} - {{ $shiftToDelete->end_time }}</p>
                                <p class="text-sm text-gray-500 mt-1">{{ $shiftToDelete->working_days_label }}</p>
                            </div>
                        @endif
                        
                        <p class="text-sm text-gray-500 mt-4">
                            {{ __('messages.delete_shift_warning') }}
                        </p>
                    </div>
