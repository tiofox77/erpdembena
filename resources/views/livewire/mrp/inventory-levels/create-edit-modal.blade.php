        ...
        <div class="relative bg-white rounded-lg shadow-xl overflow-hidden" @click.stop>
            ...
            <form wire:submit.prevent="save">
                ...
                <div class="bg-gray-50 px-4 py-3 sm:px-6 flex flex-col sm:flex-row-reverse space-y-2 sm:space-y-0 sm:space-x-2 sm:space-x-reverse">
                    @if(($editMode && auth()->user()->can('inventory_levels.edit')) || (!$editMode && auth()->user()->can('inventory_levels.create')))
                    ...
                    @else
                        <button type="button" disabled
                            class="w-full inline-flex justify-center items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-gray-400 cursor-not-allowed opacity-75"
                            title="{{ $editMode ? __('messages.no_edit_permission') : __('messages.no_permission_to_add') }}">
                            <i class="fas {{ $editMode ? 'fa-save' : 'fa-plus-circle' }} mr-2"></i>
                            {{ $editMode ? __('messages.save_changes') : __('messages.create_inventory_level') }}
                        </button>
                    @endif
                    ...
                    <button type="button" wire:click="closeModal"
                        class="w-full inline-flex justify-center items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-all duration-200">
